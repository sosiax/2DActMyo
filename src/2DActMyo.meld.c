...
#ifdef CHECK
               if (CheckFA(Lx,Ly,P[x][y][TAG],P,FA)){
                  fprintf(stderr,"Error (%s : %d ): Checking filament 3 ... \n",__FILE__, __LINE__);
                  fprintf(stderr,"prot=%d MTYPE=%d pos=[%d,%d] p=%f \n", pro, P[x][y][0] , x , y, p);
                  fprintf(stderr,"CNFA=%d CNFL=%d CNFM=%d\n", CNFA, CNFL , CNFM);
                  fprintf(stderr,"OldT=%d OldS=%d\n", OldT, OldS);
                  return 1;
         ...
         else if(P[x][y][0]!=0)
         {
            if(P[x][y][0]==1)
            {
               ...
               if((P[x][y][0]==0)&&(OldS!=0)&&(P[x][y][4]!=0)) {
                  FilOld(Lx,Ly,OldT,OldS,P,x,y,FA,FL,FM,TA);
                  if ((CNFA==37)&&(x==8)&&(y==0)&&(CNFM==42)&&(CNFL==219))
                     fprintf(stderr," ");
                  }
#ifdef CHECK
               if (CheckFA(Lx,Ly,P[x][y][TAG],P,FA)){
                  fprintf(stderr,"Error (%s : %d ): Checking filament 4 ... \n",__FILE__, __LINE__);
                  fprintf(stderr,"prot=%d MTYPE=%d pos=[%d,%d] p=%f \n", pro, P[x][y][0] , x , y, p);
                  fprintf(stderr,"CNFA=%d CNFL=%d CNFM=%d\n", CNFA, CNFL , CNFM);
                  fprintf(stderr,"OldT=%d OldS=%d\n", OldT, OldS);
                  return 1;
...
#ifdef CHECK
               if (CheckFA(Lx,Ly,P[x][y][TAG],P,FA)){
                  fprintf(stderr,"Error (%s : %d ): Checking filament 5 ... \n",__FILE__, __LINE__);
                  fprintf(stderr,"prot=%d MTYPE=%d pos=[%d,%d] p=%f \n", pro, P[x][y][0] , x , y, p);
                  fprintf(stderr,"CNFA=%d CNFL=%d CNFM=%d\n", CNFA, CNFL , CNFM);
                  fprintf(stderr,"OldT=%d OldS=%d\n", OldT, OldS);
                  return 1;
...
#ifdef CHECK
               if (CheckFA(Lx,Ly,P[x][y][TAG],P,FA)){
                  fprintf(stderr,"Error (%s : %d ): Checking filament 6 ... \n",__FILE__, __LINE__);
                  fprintf(stderr,"prot=%d MTYPE=%d pos=[%d,%d] p=%f \n", pro, P[x][y][0] , x , y, p);
                  fprintf(stderr,"CNFA=%d CNFL=%d CNFM=%d\n", CNFA, CNFL , CNFM);
                  fprintf(stderr,"OldT=%d OldS=%d\n", OldT, OldS);
                  return 1;
...
#ifdef CHECK
      if (CheckAllFA(Lx,Ly,P,FA)){
         fprintf(stderr,"Error (%s : %d ): Checking filament Final ... \n",__FILE__, __LINE__);
         fprintf(stderr,"prot=%d MTYPE=%d pos=[%d,%d] p=%f \n", pro, P[x][y][0] , x , y, p);
         fprintf(stderr,"CNFA=%d CNFL=%d CNFM=%d\n", CNFA, CNFL , CNFM);
         fprintf(stderr,"OldT=%d OldS=%d\n", OldT, OldS);
         return 1;
...
int RemoveMoleculeBonds(int Lx,int Ly,double eA,double eL,double eM,int x, int y, int*** P,double** E,int** FA,double* TA,int** FL,int** FM,int** NH){
   ...
   switch(P[x][y][MTYPE]) {
      case ACT : 
            ...
            if ((x+dx < Lx) && (y+dy < Ly) && (x+dx > 0) && (y+dy > 0) &&
                (P[x][y][TAG] == P[x+dx][y+dy][TAG])){
...
//Correccion etiquetaje
   ...
   //Primer filamento
   FA[nz][FLEN]=(ML>3)?abs(y-My0)-2:abs(y-My0)-1;          // !!! Coreccion distancia (hemos quitado otra actina anterior (antes era -1)
   if (FA[nz][FLEN] < 3) {
      ...
      FA[GLOBAL][NFACT]--;                                       //Actulizamos contador global
      }
   else {
   ...
   if (D2 < 3) {
      ...
      FA[nz][FDIR]=P[x+dx][y+dy][1];  // Direccion del nuevo filamento es la misma
      FA[nz][FLEN]=D2;                // Tamano del nuevo filamento
      FA[0][1]+=FA[nz][FLEN];         // Insertamos Tamano del nuevo filamento
      for(d=0, xi=FA[nz][X] , yi=FA[nz][Y]; d<D2 ; d++, xi+=dx , yi+=dy){
...
int DireccionXY(int Dir, int * x, int * y){
   switch(Dir) {
      ...
      default : 
         fprintf(stderr,"Error en el codigod e direccion: %d\n",Dir);
         return 1;
   }
   return 0;
}
...
int CheckFA(int Lx,int Ly,int nz, int*** P,int** FA){
   ...
   if ((nz==0)||(MD==0)) //No Filament
      return 0;

   if (DireccionXY(MD,&dx,&dy)){
      fprintf(stderr,"ERROR: Error en la direccion del filamento : %d\n",P[x][y][DIR]);
      ret=1;
      }
   tempX=x+(ML*dx);
   tempY=y+(ML*dy);
   ...
   for(d=0, xi=x , yi=y; d<ML ; d++, xi+=dx , yi+=dy){
      ...
      if (ThereIsLateral('L',Lx,Ly,P,xi,yi,&lx,&ly)){
         if (P[xi][yi][TAG] != P[lx][ly][TAG]){
            fprintf(stderr,"ERROR: Lateral L of element %d of filament %d (%d,%d - DIR %d[%d,%d]) is not same TAG (%d!=%d)\n",d,nz,x,y,MD,dx,dy,P[xi][yi][TAG],P[lx][ly][TAG]);
            fprintf(stderr,"ERROR: Lateral type %d (%d,%d)\n",P[lx][ly][MTYPE],lx,ly);
            ret=1;
            }
         }
      if (ThereIsLateral('R',Lx,Ly,P,xi,yi,&lx,&ly)){
         if (P[x][y][TAG] != P[lx][ly][TAG]){
            fprintf(stderr,"ERROR: Lateral R of element %d of filament %d (%d,%d - DIR %d[%d,%d]) is not same TAG (%d!=%d)\n",d,nz,x,y,MD,dx,dy,P[x][y][TAG],P[lx][ly][TAG]);
            fprintf(stderr,"ERROR: Lateral type %d (%d,%d)\n",P[lx][ly][MTYPE],lx,ly);
            ret=1;
            }
         }
...
int CheckAllFA(int Lx,int Ly, int*** P,int** FA){
   int nz=0,ret=0;
   int x,y;
